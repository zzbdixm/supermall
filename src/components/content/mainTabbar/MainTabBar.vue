<template>
  <tab-bar>
      <tab-bar-item path="/home" activeColor="deeppink">
        <template v-slot:item-icon>
          <img class="item-icon" src="~assets/img/tabbar/home.svg" alt="">
        </template>
        <template v-slot:item-icon-active>
          <img class="item-icon-active" src="~assets/img/tabbar/home_active.svg" alt="">
        </template>
        <template v-slot:item-text>
          <div class="item-text">首页</div>
        </template>
      </tab-bar-item>
      <tab-bar-item path="/category" activeColor="deeppink">
        <template v-slot:item-icon>
          <img class="item-icon" src="~assets/img/tabbar/category.svg" alt="">
        </template>
        <template v-slot:item-icon-active>
          <img class="item-icon-active" src="~assets/img/tabbar/category_active.svg" alt="">
        </template>
        <template v-slot:item-text>
          <div class="item-text">分类</div>
        </template>
      </tab-bar-item>
      <tab-bar-item path="/cart" activeColor="deeppink">
        <template v-slot:item-icon>
          <img class="item-icon" src="~assets/img/tabbar/shopcart.svg" alt="">
        </template>
        <template v-slot:item-icon-active>
          <img class="item-icon-active" src="~assets/img/tabbar/shopcart_active.svg" alt="">
        </template>
        <template v-slot:item-text>
          <div class="item-text">购物车</div>
        </template>
      </tab-bar-item>
      <tab-bar-item path="/profile" activeColor="deeppink">
        <template v-slot:item-icon>
            <img class="item-icon" src="~assets/img/tabbar/profile.svg" alt="">
          </template>
          <template v-slot:item-icon-active>
            <img class="item-icon-active" src="~assets/img/tabbar/profile_active.svg" alt="">
          </template>
          <template v-slot:item-text>
            <div class="item-text">我的</div>
          </template>
      </tab-bar-item> 
    </tab-bar>
</template>

<script>
import TabBar from 'components/common/tabbar/TabBar'
import TabBarItem from 'components/common/tabbar/TabBarItem'

export default {
  name:'MainTabbar',
  components:{
    TabBar,
    TabBarItem
  }
}
</script>

<style  scoped>
.item-icon,.item-icon-active{
    height: 24px;
    width:24px;
    margin-top: 3px;
    vertical-align: top;
    margin-bottom: 2px;
  }
</style>
